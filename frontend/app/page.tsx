...
export default function PPSDashboard() {
  ...
  const getFilteredStats = () => {
    ...
    // Calculate total antibiotics from filtered patients
    const totalAntibiotics = filteredPatients.reduce((total, patient) => {
      return total + (patient.patient_number_antibiotics || 0);
    }, 0);
  ...
  // Calculate AWaRe categorization stats from filtered patients
  const getFilteredAwareStats = () => {
    const filteredPatients = getFilteredPatients();
    ...
    // Filter antibiotics to only those belonging to filtered patients
    // Try matching by parent_key (which might be patient ID) or by patient_id if available
    const filteredAntibiotics = allAntibiotics.filter((antibiotic) => {
      // Check if antibiotic belongs to any filtered patient
      // parent_key might match patient.id or patient.instance_id
      return filteredPatients.some(
        (patient) =>
          antibiotic.parent_key === patient.id ||
          antibiotic.parent_key === patient.instance_id ||
          (antibiotic as any).patient_id === patient.id
      );
    });

    // If no filters are applied, use all antibiotics
    const antibioticsToUse =
      filteredPatientIds.size === allPatients.length
        ? allAntibiotics
        : filteredAntibiotics;

    // Count antibiotics by AWaRe classification
    const awareCounts = {
      Access: 0,
      Watch: 0,
      Reserve: 0,
    ...
    antibioticsToUse.forEach((antibiotic) => {
      const classification =
        antibiotic.antibiotic_aware_classification?.trim() || '';
      ...
      if (normalized === 'access') {
        ...
        awareCounts.Watch++;
      } else if (normalized === 'reserve') {
        awareCounts.Reserve++;
      } else {
        awareCounts.Unclassified++;
      }
    });

    const totalAntibiotics = antibioticsToUse.length;

    // Calculate percentages
    const calculatePercentage = (count: number) => {
      return totalAntibiotics > 0 ? (count / totalAntibiotics) * 100 : 0;
    };

    return {
      access: {
        count: awareCounts.Access,
        percentage: calculatePercentage(awareCounts.Access),
        description: 'First choice antibiotics',
      },
      watch: {
        count: awareCounts.Watch,
        percentage: calculatePercentage(awareCounts.Watch),
        description: 'Second choice antibiotics',
      },
      reserve: {
        count: awareCounts.Reserve,
        percentage: calculatePercentage(awareCounts.Reserve),
        description: 'Last resort antibiotics',
      },
      unclassified: {
        count: awareCounts.Unclassified,
        percentage: calculatePercentage(awareCounts.Unclassified),
      ...
      try {
        ...
        const [
          ...
          PPSApi.getSpecimenStats({
            start_date:
              selectedFromDate !== 'all' ? selectedFromDate : undefined,
            end_date: selectedToDate !== 'all' ? selectedToDate : undefined,
            region: selectedRegion !== 'all' ? selectedRegion : undefined,
            district: selectedDistrict !== 'all' ? selectedDistrict : undefined,
            subcounty:
              selectedSubCounty !== 'all' ? selectedSubCounty : undefined,
            facility: selectedFacility !== 'all' ? selectedFacility : undefined,
            level:
              selectedLevelOfCare !== 'all' ? selectedLevelOfCare : undefined,
            ownership:
              selectedOwnership !== 'all' ? selectedOwnership : undefined,
          }),
          PPSApi.getPatients({ limit: 999999 }),
          PPSApi.getAntibiotics({ limit: 999999 }),
          ...
          PPSApi.getCultureMetric({
            ...
            ownership:
              selectedOwnership !== 'all' ? selectedOwnership : undefined,
            ward: selectedWardName !== 'all' ? selectedWardName : undefined,
          }),
        ...
        setAllAntibiotics(allAntibioticsRes.data || []);
  ...
  useEffect(() => {
    const fetchAppropriateDiagnosis = async () => {
      try {
        const appropriateDiagnosisRes = await PPSApi.getAppropriateDiagnosis({
          start_date: selectedFromDate !== 'all' ? selectedFromDate : undefined,
          end_date: selectedToDate !== 'all' ? selectedToDate : undefined,
          region: selectedRegion !== 'all' ? selectedRegion : undefined,
          district: selectedDistrict !== 'all' ? selectedDistrict : undefined,
          subcounty:
  ...
  useEffect(() => {
    const fetchBasicMetrics = async () => {
      try {
        ...
        console.log('Filtered Basic Metrics API Response:', allBasicMetrics);
        setAllBasicMetrics(allBasicMetrics);
      } catch (err) {
        console.error('Error fetching filtered basic metrics data:', err);
      }
    };
  ...
  useEffect(() => {
    const fetchCultureMetrics = async () => {
      try {
        ...
        console.error('Error fetching filtered culture metrics data:', err);
      }
    };
  ...
  useEffect(() => {
    const fetchGenericMetrics = async () => {
      try {
        ...
        console.error('Error fetching filtered generic metrics data:', err);
      }
    };
  ...
  useEffect(() => {
    const fetchGuidelineMetrics = async () => {
      try {
        ...
        console.error('Error fetching filtered guideline metrics data:', err);
      }
    };
  ...
  useEffect(() => {
    const fetchIndicators = async () => {
      try {
        ...
        console.log('Filtered Indicators API Response:', allIndicators);
        setAllIndicators(allIndicators);
      } catch (err) {
        console.error('Error fetching filtered indicators data:', err);
      }
    };
  ...
  useEffect(() => {
    const fetchPatientDaysMetrics = async () => {
      try {
        const allPatientDaysMetrics = await PPSApi.getPatientDaysMetric({
          start_date: selectedFromDate !== 'all' ? selectedFromDate : undefined,
          end_date: selectedToDate !== 'all' ? selectedToDate : undefined,
          region: selectedRegion !== 'all' ? selectedRegion : undefined,
          district: selectedDistrict !== 'all' ? selectedDistrict : undefined,
  ...
  useEffect(() => {
    const fetchSpecimenStats = async () => {
      try {
        ...
        console.log('Filtered Specimen Stats API Response:', specimenStatsRes);
        setSpecimenStats(specimenStatsRes);
      } catch (err) {
        console.error('Error fetching filtered specimen stats data:', err);
      }
    };
  ...
  const filteredPatients = getFilteredPatients();
  const filteredAwareStats = getFilteredAwareStats();
  ...
  return (
    <div className="h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      ...
      >
        {/* Filter Toggle Button - shown when filters are hidden */}
        {!filtersVisible && (
          <div className="px-6 pt-8 py-4 bg-white/50 dark:bg-slate-900/50 border-b border-slate-200/50 dark:border-slate-800/50">
        ...
        {/* Filters Section - shown when filtersVisible is true */}
        {filtersVisible && (
          <div className="px-4 pt-6 py-4 bg-white/50 dark:bg-slate-900/50 border-b border-slate-200/50 dark:border-slate-800/50">
              ...
              <div className="flex items-center gap-2 flex-wrap py-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    min={
                      selectedFromDate === 'all' ? undefined : selectedFromDate
                    }
                    className="px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                  />
                </div>
                ...
                {/* Filter Toggle Button - hide filters */}
                <Button
                  variant="default"
                  size="sm"
            ...
            <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950/50 dark:to-blue-900/50 border-blue-200/50 dark:border-blue-800/50">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-blue-900 dark:text-blue-100">
                  {loading
                    ? '...'
                    : [...new Set(filteredPatients.map((p) => p.facility))]
                        .filter(Boolean)
            ...
            <Card className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-950/50 dark:to-purple-900/50 border-purple-200/50 dark:border-purple-800/50">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-purple-900 dark:text-purple-100">
                  {loading
                    ? '...'
                    : filteredStats?.total_antibiotics?.toLocaleString() || '0'}
                </div>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                      {loading
                        ...
                        ? (
                            filteredStats.total_antibiotics /
                            filteredStats.patients_on_antibiotic
                          ).toFixed(2)
                        : '0.0'}
                    </div>
                    <p className="text-xs text-blue-600 dark:text-blue-400">
                      N: {filteredStats?.total_antibiotics || 0} / D:{' '}
                      {filteredStats?.patients_on_antibiotic || 0}
                    </p>
                  </CardContent>
                </Card>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <span className="text-xs font-bold text-green-700 dark:text-green-300">
                                  {awareStats?.access?.percentage?.toFixed(1) ||
                                    0}
                                  %
                                </span>
                              </div>
                              <Progress
                                value={awareStats?.access?.percentage || 0}
                                className="h-1.5"
                              />
                              <div className="text-xs text-green-500 dark:text-green-500">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                <span className="text-xs font-bold text-yellow-700 dark:text-yellow-300">
                                  {awareStats?.watch?.percentage?.toFixed(1) ||
                                    0}
                                  %
                                </span>
                              </div>
                              <Progress
                                value={awareStats?.watch?.percentage || 0}
                                className="h-1.5"
                              />
                              ...
                              <Progress
                                value={awareStats?.reserve?.percentage || 0}
                                className="h-1.5"
                              />
                              ...
                              <Progress
                                value={
                                  awareStats?.unclassified?.percentage || 0
                                }
                                className="h-1.5"
                              />
                          ...
                          <CardContent className="p-4">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-green-700 dark:text-green-300">
                                {filteredAwareStats?.access?.percentage?.toFixed(
                                  1
                                ) || 0}
                                %
                              </div>
                              ...
                              <div className="text-xs text-green-500 dark:text-green-500 mt-1">
                                {filteredAwareStats?.access?.description ||
                                  'First choice antibiotics'}
                              </div>
                              <div className="text-xs text-green-500 dark:text-green-500 mt-1">
                                {filteredAwareStats?.access?.count?.toLocaleString() ||
                                  0}{' '}
                                antibiotics
                              </div>
                            </div>
                          ...
                          <CardContent className="p-4">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-amber-700 dark:text-amber-300">
                                {filteredAwareStats?.watch?.percentage?.toFixed(
                                  1
                                ) || 0}
                                %
                              </div>
                              ...
                              <div className="text-xs text-amber-500 dark:text-amber-500 mt-1">
                                {filteredAwareStats?.watch?.description ||
                                  'Second choice antibiotics'}
                              </div>
                              <div className="text-xs text-amber-500 dark:text-amber-500 mt-1">
                                {filteredAwareStats?.watch?.count?.toLocaleString() ||
                                  0}{' '}
                                antibiotics
                              </div>
                            </div>
                          ...
                          <CardContent className="p-4">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-red-700 dark:text-red-300">
                                {filteredAwareStats?.reserve?.percentage?.toFixed(
                                  1
                                ) || 0}
                                %
                              </div>
                              ...
                              <div className="text-xs text-red-500 dark:text-red-500 mt-1">
                                {filteredAwareStats?.reserve?.description ||
                                  'Last resort antibiotics'}
                              </div>
                              <div className="text-xs text-red-500 dark:text-red-500 mt-1">
                                {filteredAwareStats?.reserve?.count?.toLocaleString() ||
                                  0}{' '}
                                antibiotics
                              </div>
                            </div>
                          ...
                          <CardContent className="p-4">
                            <div className="text-center">
                              <div className="text-2xl font-bold text-gray-700 dark:text-gray-300">
                                {filteredAwareStats?.unclassified?.percentage?.toFixed(
                                  1
                                ) || 0}
                                %
                              </div>
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                                {filteredAwareStats?.unclassified
                                  ?.description || 'Not categorized'}
                              </div>
                              <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                                {filteredAwareStats?.unclassified?.count?.toLocaleString() ||
                                  0}{' '}
                                antibiotics
                              </div>
                            </div>
